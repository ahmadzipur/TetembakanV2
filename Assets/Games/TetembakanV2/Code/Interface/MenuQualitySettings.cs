...
namespace InfimaGames.LowPolyShooterPack.Interface
{
    ...
    public class MenuQualitySettings : Element
    {
        ...
        [Title(label: "Serial Settings")]

        [SerializeField]
        private TMP_Dropdown dropdownSerialPort;
        ...
        private void Start()
        {
            ...
            LoadSerialPorts();
            dropdownSerialPort.onValueChanged.AddListener(OnSerialPortChanged);
        ...
        {
            if (dropdownSerialPort != null)
                dropdownSerialPort.onValueChanged.RemoveListener(OnSerialPortChanged);
        }
        ...
        public void LoadSerialPorts()
        {
            ...
            dropdownSerialPort.AddOptions(ports);

            // Load saved port
            string savedPort = PlayerPrefs.GetString("GYRO_COM_PORT", "");
            int index = ports.IndexOf(savedPort);
            dropdownSerialPort.value = index >= 0 ? index : 0;
        }

        public void ApplySerialPort()
        {
            string selectedPort = dropdownSerialPort.options[dropdownSerialPort.value].text;

            if (selectedPort == "NO PORT")
                return;

            PlayerPrefs.SetString("GYRO_COM_PORT", selectedPort);
            PlayerPrefs.Save();

            Debug.Log($"[SETTINGS] Serial Port set to {selectedPort}");
        }

        private void OnSerialPortChanged(int index)
        {
            ApplySerialPort();
        }
